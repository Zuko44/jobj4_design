...
select car_bodies.name as bodies, cars.name from car_bodies
left join cars on car_bodies.id = cars.body_id
where cars.name is null;

select car_engines.name as engine, cars.name from car_engines
left join cars on car_engines.id = cars.engine_id
where cars.name is null;

select car_transmissions.name as transmission, cars.name from car_transmissions
